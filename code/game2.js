...
var actorChars = {
  "@": Player,
  "r": RedKey, // A RedKey will wobble up and down
  "b": BlueKey,
  "y": YellowKey,
  "v": GreenKey,
  "=": Enemy  
...
function RedKey(pos) {
  ...
  this.size = new Vector(1.1, 0.4);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
RedKey.prototype.type = "redkey";

function BlueKey(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0, 0.1));
  this.size = new Vector(1.1, 0.4);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
BlueKey.prototype.type = "bluekey";

function YellowKey(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0, 0.1));
  this.size = new Vector(1.1, 0.4);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
YellowKey.prototype.type = "yellowkey";

function GreenKey(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0, 0.1));
  this.size = new Vector(1.1, 0.4);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
GreenKey.prototype.type = "greenkey";

// spikes is initialized based on the character, but otherwise has a
// size and position
function Enemy(pos, ch) {
  this.pos = pos;
  this.size = new Vector(1, 1.9);
  if (ch == "=") {
    // Horizontal spikes
...
RedKey.prototype.act = function(step) {
...
BlueKey.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
YellowKey.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
GreenKey.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  // Handle spikes by calling playerTouched
  if (obstacle)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches spikes and the player hasn't won
  // Player loses
  if (type == "spikes" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "enemy" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "redkey" || "blueKey" || "yellowkey" || "greenkey") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    // If there aren't any Keys left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "redkey" && "blueKey" && "yellowkey" && "greenkey";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
        ...
        alert("You have just won Grave Robber!");
    });
  }
  startLevel(0);
}
